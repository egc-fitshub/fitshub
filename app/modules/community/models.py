...
class CommunityDataSet(db.Model):
    ...
    status = db.Column(
        SQLAlchemyEnum(CommunityDataSetStatus), 
        nullable=False, 
        default=CommunityDataSetStatus.PENDING
    )
    ...
    def __repr__(self):
        return (f"<CommunityDataSet Community={self.community_id} "
                f"Dataset={self.dataset_id} Status={self.status.value}>")
...
class Community(db.Model):
    ...
    curators = db.relationship('User', secondary='community_curator_association', 
                               lazy='dynamic', backref=db.backref('curated_communities', lazy='dynamic'))
    ...
    def datasets(self):
        return DataSet.query.join(CommunityDataSet).filter(
            CommunityDataSet.community_id == self.id,
            CommunityDataSet.status == CommunityDataSetStatus.ACCEPTED
        )
